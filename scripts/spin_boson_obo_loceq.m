...
gamma_B = [1.0] ;
lambda_B = [1.0] ;
Omega_B = 0.25 * gamma_B ;
epsilon = 0.0*lambda_B ;
k_B_T = 1/beta ;

% define the system Hamiltonian and the system-bath coupling
d_hilb = 2 ;
d_liou = d_hilb^2 ;
H_sys = sparse([0,Delta;Delta,epsilon+lambda_B]) ;
V = {sparse([0,0;0,1])} ;
...
Gamma_cut = 60.0 * Omega_B(1) ;
fprintf('Creating HEOM generator... ') ;
[L_heom,ado_indices] = constructScaledHEOMGeneratorFreqTruncOBO(H_sys,V,gamma_B,lambda_B,Omega_B,beta,Gamma_cut) ;
