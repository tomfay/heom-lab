...
deltaE_min = -500 ;
n_deltaE = 20 ;
deltaEs = linspace(deltaE_min,deltaE_max,n_deltaE);

for k_deltaE = 1:n_deltaE
...
% E_LE = E_LE(chla_inds) ;
E_LE = E_LE + deltaEs(k_deltaE) ;
E_LE([11,12]) = E_LE([11,12]) - deltaEs(k_deltaE) ;
E_LE'
E_LE_min = min(E_LE) ;
