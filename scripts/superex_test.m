...
full_system.baths = [full_system.baths,...
    ...
    "spectral_density","debye","omega_D",omega_D,"lambda_D",lambda_D)}] ;
full_system.Vs = {} ;
full_system.Vs{1} = {[[1,0];[0,0]],[[1]]} ;
full_system.Vs{2} = {[[0,0];[0,1]],[[0]]} ;
full_system.beta = beta ;

% information about the different blocks
full_system.block_coupling = struct() ;
full_system.block_coupling.E_blocks = [0,E_3] ;
full_system.block_coupling.coupled_blocks = [[1,2]] ;
full_system.block_coupling.coupling_matrices = {[Gamma_CT1;Gamma_CT2]} ;
full_system.block_coupling.coupling_baths = {} ;
full_system.block_coupling.coupling_baths{1} = ...
    {struct("spectral_density","debye","omega_D",omega_CT,"lambda_D",lambda_CT,...
    "n_modes",n_modes)} ;
full_system.block_coupling.n_ts = [n_t] ;
full_system.block_coupling.t_maxs = [t_max] ;
% set up the dynamics struct
heom_dynamics = struct() ;
% set the initial condition
heom_dynamics.rho_0_sys = {[[1,0];[0,0]],[[0]]} ;
