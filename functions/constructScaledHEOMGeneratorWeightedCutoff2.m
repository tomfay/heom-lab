function [L_heom,ado_indices] = constructScaledHEOMGeneratorWeightedCutoff2(H_sys,V,gamma_Ds,lambda_Ds,beta,epsilon_cut)

% first find the maximum M, highest Matsubara mode to be treated explicitly
% given the value of Gamma_cut & beta
M = findMaxMjWeightedCutoff2(epsilon_cut,gamma_Ds, lambda_Ds,beta) ;

% find the set of frequencies and expansion coefficients for each bath
% using the Matsubara expansion up to mode M
[nus_array,cs_array] = generateNusAndCs(gamma_Ds,lambda_Ds,beta,M) ;
...
[ado_indices,ado_gammas,lower_indices,upper_indices,coupled_mode_indices,truncated_coupled_modes] = generateHierarchyWeightedCutoff2Trunc(epsilon_cut,nus,cs);
...
for j = 1:n_baths
    ...
    R_j = 2.0*lambda_Ds(j)/(beta*gamma_Ds(j)) - lambda_Ds(j)*cot(beta*gamma_Ds(j)/2) - sum(cs_array(j,2:end)./nus_array(j,2:end)) ;
...
for r = 1:n_couplings
    J = lower_indices(r) ; % shallower in hierarchy
    K = upper_indices(r) ; % deeper in hierarchy
    jk_coup = coupled_mode_indices(r) ;
    j_coup = ceil(jk_coup/(M+1)) ; % get the bath index that is coupling J & K
    k_coup = jk_coup - j_coup*(M+1) ; % Matsubara mode index, k=0 is non-Matsubara term
    J_block = ((J-1)*(d_liou)+1):(J*d_liou) ;
    ...
    L_heom(K_block,J_block) = L_heom(K_block,J_block) ...
        ...
        + (1.0i*conj(c_jk_coup)*sqrt(n_jk_coup/abs(c_jk_coup))) * V_R{j_coup} ; 
end
...
% THIS IS THE main truncation used
    L_heom(J_block,J_block) = L_heom(J_block,J_block) -(1.0i/(ado_gammas(J)+nus(jk_term))) * V_comm{j_term}*((-1.0i*c_jk_term*(n_jk_term+1)) * V_L{j_term} ...
        + (1.0i*conj(c_jk_term)*(n_jk_term+1)) * V_R{j_term} ) ;
