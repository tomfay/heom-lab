function [ado_indices,ado_gammas,lower_indices,upper_indices,coupled_mode_indices,truncated_coupled_modes,ado_indices_term,modes_term,term_indices] = generateHierarchyLindoyTrunc(L_max,L_min,nus)
...
nu_min = min(abs(nus)) ;
Gamma = L_max / L_min ;
weights = min([[abs(nus);(L_max/L_min) * nu_min * ones(size(nus))]],[],1) ;
% nus
% weights
% nu_min
...
for L = 1:L_max
    ...
    for k = 1:n_modes
        for r = 1:n_lower
            ...
            is_included = sum(weights.*ado_indices_new) < (L_max *nu_min);
