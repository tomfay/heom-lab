function [ado_indices,ado_gammas,lower_indices,upper_indices,coupled_mode_indices,truncated_coupled_modes,ado_indices_term,modes_term,term_indices] = generateHierarchyWeightedCutoffTrunc(L_cut,nus,cs)
...
L_max_modes = floor(L_cut*sqrt(abs(cs))./abs(nus)) ;
L_max = max(L_max_modes) ;

% mode weights in calculating
mode_weights = abs(nus) ./sqrt(abs(cs)) ;
...
for L = 1:L_max
    ...
    for k = 1:n_modes
        for r = 1:n_lower
            ...
            cutoff_func = sum(ado_indices_new.*mode_weights) ;
            is_included_in_hierarchy = (cutoff_func<L_cut) ;
            % if it is a new index set, add it to the list and store the
            % coupling
            if ((~is_member) & is_included_in_hierarchy)
            ...
            elseif is_member
                ...
                upper_indices = [upper_indices; K] ;
                coupled_mode_indices = [coupled_mode_indices ; k] ;
            elseif (~is_included_in_hierarchy)
            ...
            if (~is_member_term & ~is_included_in_hierarchy)
            ...
            elseif (is_member_term & ~is_included_in_hierarchy)
