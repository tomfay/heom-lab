function [ado_indices,ado_gammas,lower_indices,upper_indices,coupled_mode_indices,truncated_coupled_modes,ado_indices_term,modes_term,term_indices] = generateHierarchyCouplingWeightedCutoffTrunc(L_cut,p,nus,cs)
...
% cut off criterion is sum_jk (n_jk (nu_jk/sqrt(|c_jk|)))^p < L_cut^p
mode_weights = (abs(nus) ./sqrt(abs(cs))) ;

% calculate L_max for the hierarchy
L_max_modes = floor(L_cut ./ mode_weights) ;
    ...
    for k = 1:n_modes
        for r = 1:n_lower
            ...
            elseif (is_member_term & ~is_included)
                ...
                mode_term{J_term} = [mode_term{J_term},k] ;
