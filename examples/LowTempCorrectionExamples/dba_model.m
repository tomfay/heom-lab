...
Gamma_cut = omega_D * 10.01 ;
% matrices of system observable operators to be returned, sigma_x, sigma_y
% sigma_z, and 1
O_sys = {diag([1 0 0]),diag([0 1 0]), diag([0 0 1])} ;
O_sys = [O_sys , {zeros(3),zeros(3),zeros(3)}] ;
O_sys{4}(1,2) = 1 ;
O_sys{5}(1,3) = 1 ;
O_sys{6}(2,3) = 1 ;

% initial state of the system
rho_0_sys = diag([1 0 0 ]) ;
...
full_system.H_sys = [[0,Delta,0];
                     [Delta,epsilon_B + lambda_D*0.25,Delta];
                     [0,Delta,epsilon+lambda_D ]];
% baths is a cell array of structs describign each bath
full_system.baths = {struct("V",[[0,0,0];[0,0.5,0];[0,0,1]],...
